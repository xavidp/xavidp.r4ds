...
Casi todas las funciones que usarás en este libro producen tibbles ya que éstos son una de las características principales de tidyverse. La mayoría de los paquetes de R suelen usar data frames clásicos, así que quizás quieras convertir un data frame en un tibble. Lo puedes conseguir con `as_tibble()`:

```{r}
as_tibble(iris)
```

Puedes crear un nuevo tibble a partir de vectores individuales con `tibble()`. `tibble()` automáticamente recicla vectores de longitud 1 y te permite usar variables creadas dentro de la propia función, como se muestra abajo.
...
Otra forma de crear un tibble es con `tribble()`, que es una abreviación de tibble **tr**anspuesto. `tribble()` está pensado para data entry en el código: los nombres de las columnas se definen con fórmulas (comienzan con `~`), y cada entrada está separada por comas. Esto permite escribir pocos datos de manera legible.
...
Usualmente agrego un comentario (esta línea debe iniciar con `#`) para dejar en claro cuál es el encabezado.
...
Los tibbles tienen un método de impresión en la consola refinado que sólo muestra las primeras 10 filas y solo las columnas que entran en la pantalla. Esto facilita mucho trabajar con bases de datos grandes. Además del nombre, cada columna muestra su tipo, una característica útil tomada de `str()`
...
También puedes controlar el características de impresión, modificando las opciones que estan determinadas por default.
...
La opción final es usar el visualizador de datos de RStudio para obtener una versión interactiva del data frame completo. Esto también es útil al final de una cadena larga de manipulaciones.
...
Hasta ahora todas las herramientas que aprendiste funcionan con el data frame completo. Si quieres recuperar una variable individual, necesitas algunas herramientas nuevas: `$` y `[[`. `[[` permiten extraer variables usando su nombre o posición; `$` solo extrae mediante el nombre, pero permite tipear un poco menos.
...
Para usarlos con un pipe, necesitas el símbolo especial `.`:
...
La principal razón de que algunas funciones previas no funcionen con tibbles es la función `[`. En este libro no usamos mucho `[` porque `dplyr::filter()` y `dplyr::select()` resuelven los mismos problemas con un código más claro (pero aprenderás un poco sobre ello en [vector subsetting](#vector-subsetting) XXX). Con los data frames de R base, `[`  a veces devuelve un data frame y a veces devuelve un vector. Con tibbles, `[` siempre devuelve otro tibble.

## Ejercicios


1. ¿Cómo puedes saber si un objeto es un tibble? (Sugerencia: imprime `mtcars` en consola, que es un data frame clásico).

1. Compara y contrasta las siguientes operaciones en un `data.frame` y en un tibble equivalente. ¿Qué es diferente? ¿Por qué podría causarte problemas el comportamiento por default del data frame?
