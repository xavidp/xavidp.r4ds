...
1.  En lugar de eso, recomiendo reemplazar los valores inusuales con valores faltantes.
    La manera más fácil de hacerlo es usar `mutate()` (del inglés __mutar__) para reemplazar la variable
...
1.  ¿Qué efecto tiene usar `na.rm = TRUE` en `mean()` (del inglés __promedio__) y `sum()`(del inglés __sumar__)?
...
Otra alternativa para mostrar la distribución de una variable continua agrupada en relación a otra variable discreta es el diagrama de caja ( también conocido como diagrama de cajas y bigotes; __boxplot__, en inglés). Un **diagrama de caja** es un tipo de representación visual para la distribución de valores popular entre las personas que se dedican a la estadística. Cada diagrama de caja está integrado por:

* Una caja que comprende desde el percentil 25avo de la distribución hasta el 75avo percentil, 
  distancia que se conoce como rango intercuartílico (RIC; __interquartile range__ o __IQR__ en inglés). 
...
* Los puntos que representan observaciones que se encuentran a más de 1.5 RIC
...
Si los nombres de tus variables son muy largos, `geom_boxplot()` funcionará mejor si giras el gráfico 90°. Puedes hacer esto agregando `coord_flip()` (del inglés __coordinate flip__, como para "voltear" las coordenadas).
...
Para visualizar la variación entre variables discretas, deberás contar el número de observaciones de cada combinación. Una manera de hacerlo es empleando la función integrada `geom_count()` (del inglés __count__, contar):
...
Pero agregar transparencia puede ser difícil para conjuntos de datos que sean muy grandes. Otra solución es modificar el parámetro _bin_ (del inglés "contenedor", mejor traducido al español en nuestro contexto como "rango" o "unidad"). Anteriormente usaste `geom_histogram()` y `geom_freqpoly()` para segmentar una variable en rangos de manera unidimensional. Ahora aprenderás a usar `geom_bin2d()` y `geom_hex()` para hacerlo en dos dimensiones.
...
Si quieres aprender más acerca de la mecánica de `ggplot2`, te recomendaría consultar una copia del libro de ggplot2: <https://amzn.com/331924275X> (disponible en inglés). Fue actualizado recientemente, de manera que incluye código para dplyr y tidyr, y tiene mucho más espacio para explorar todas las facetas de visualización. Desafortunadamente el libro no está disponible sin costo, pero si tienes alguna relación con una universidad es probable que puedas obtener una versión electrónica del libro sin costo mediante SpringerLink.
