...
Al igual que en muchas otras funciones de R, los valores perdidos son contagiosos. Si quieres que se impriman como `"NA"`, utiliza `str_replace_na()` (_replace_ = remplazar):
...
Aprenderás más sobre listas en el capítulo [Listas](#lists) y en [Iteración](#iteration).
...
En vez de dividir una cadena según patrones, puedes hacerlo definiendo límites: caracter, línea, oración o palabra. Para ello, puedes utilizar la función `boundary()` (_límite_). En el siguiente ejemplo la división se hace por palabra (_word_):
...
*   `fixed()`: busca una coincidencia exacta de la secuencia de bytes especificada.
    ...
    Se _renderean_ de manera idéntica, pero como están definidas de manera distinta, 
...
*   `dir()` entrega una lista con todos los archivos en un directorio. El argumento
    ...
    Por ejemplo, puedes encontrar todos los archivo de R Markdown en el 
